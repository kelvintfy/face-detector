...
class REKOG {
  ...
  async searchImagesWithMatchedFace(collectionId, faceId) {
    ...
    const response = await this.client.send(command);
    if (response.FaceMatches < 1) throw new Error(`No matching image with face id: ${faceId}`);

    const matchingImageIds = response.FaceMatches.map((obj) => obj.Face.ImageId);
